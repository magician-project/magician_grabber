...
class SharedMemoryServer:
    ...
    def load_library(self):
        # Resolve the .so relative to this file so the script can be run from any directory
        lib_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "libSharedMemoryVideoBuffers.so")
        lib = ctypes.CDLL(lib_path, mode=ctypes.RTLD_GLOBAL)

        lib.allocateLocalMapping.restype = c_void_p                          # plain pointer, not pointer-to-pointer
        ...
        lib.unmapLocalMappingItem.restype   = c_int
        ...
        lib.writeVideoFrameToImage.restype  = c_int
        lib.mapRemoteToLocal.argtypes = [c_void_p, c_void_p, c_uint]
        lib.mapRemoteToLocal.restype  = c_int
    ...
    def cleanup(self):
        max_buffers = self.get_shared_memory_context_max_buffers()
    ...
    def run(self):
        try:
            while True:
                ...
                # Snapshot the count once to avoid a TOCTOU race between the
                # emptiness check and the iteration below.
                num_buffers = self.get_shared_memory_context_number_of_buffers()
