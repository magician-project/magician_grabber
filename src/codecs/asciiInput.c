...
unsigned char * ReadASCIIRaw(unsigned char * buffer ,const char * filename,unsigned int *width,unsigned int *height,unsigned long * timestamp,int packed)
{
    ...
    if (fp!=0 )
    {
        *width=0; *height=0; *timestamp=0;
        ...
        }  else
        {
          ...
          for (i=0; i<3; i++)
          {
           unsigned char * pixelsPtr = pixels+i;
           for (y=0; y<*height; y++)
           {
            for (x=0; x<*width; x++)
             {
